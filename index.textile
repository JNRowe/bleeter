...
@bleeter@ is a nasty little client for "twitter":http://twitter.com, that currently very much exists in a *Works For Me* state.  It isn't intended to be used by others, but perhaps others will find it useful.

All it does is fetch your friends timeline and display notification popups for new tweets.  If your system's notification daemon supports adding actions to the popups then you'll be able to take a number of actions by clicking the buttons on the popup.
...
You can download releases from the "downloads page":http://github.com/JNRowe/bleeter/downloads.
...
If you're using "Gentoo":http://www.gentoo.org all the dependencies are available from either the main tree or "my overlay":http://github.com/JNRowe/misc-overlay.

@bleeter@ doesn't require installation to work as it is only a single "Python":http://www.python.org/ script.
...
The "Sphinx":http://sphinx.pocoo.org/ generated documentation can be read "online":sphinx/.

You can view the "current bug status":ditz/ without installing "ditz":http://ditz.rubyforge.org/.
...
If you have made changes that you wish to contribute back then GitHub pull requests are the preferred method.  Patches by email are also welcome, but pull requests are likely to be handled faster.
